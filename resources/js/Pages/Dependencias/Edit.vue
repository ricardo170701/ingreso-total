            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-slate-600 dark:text-slate-400">
                        Modifica la información de la dependencia.
                    </p>
                </div>
                <Link
...
const form = useForm({
    ...
    piso_id: props.dependencia.piso_id || null,
...
const eliminar = () => {
    if (
        !confirm(
            `¿Estás seguro de eliminar la dependencia "${props.dependencia.nombre}"?`
        )
    )
        return;
    router.delete(
        route("dependencias.destroy", { dependencia: props.dependencia.id })
    );
};
</script>