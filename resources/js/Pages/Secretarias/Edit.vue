            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-slate-600 dark:text-slate-400">
                        Modifica la información de la secretaría.
                    </p>
                </div>
                <Link
            ...
            >
                ...
                <div class="p-6">
                    <p class="text-sm text-slate-700 dark:text-slate-300 mb-4">
                        ¿Estás seguro de que deseas editar la secretaría
                        <strong class="text-slate-900 dark:text-slate-100">{{ secretaria.nombre }}</strong>?
                    </p>
...
const form = useForm({
    ...
    piso_id: props.secretaria.piso_id || null,
    descripcion: props.secretaria.descripcion || "",
    activo: !!props.secretaria.activo,
});

const confirmSubmit = () => {
    showConfirmModal.value = false;
    form.put(route("secretarias.update", { secretaria: props.secretaria.id }));
};

const eliminar = () => {
    if (
        !confirm(
            `¿Estás seguro de eliminar la secretaría "${props.secretaria.nombre}"?`
        )
    )
        return;
    router.delete(
        route("secretarias.destroy", { secretaria: props.secretaria.id })
    );
};
</script>
